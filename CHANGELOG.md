...
- Live ("rich") editing: the cursor's line shows raw Markdown, every other line
  ...
  `markdownCoIn.richEditing`. Markers are hidden, never removed, so the
  document on disk is unchanged.
...
- Formatting toolbar with bold, italic, strikethrough, inline code, H1–H3,
...
- Export as HTML, producing a single self-contained file with no external references.
